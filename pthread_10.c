...
void* watch_count(void* arg)
{
	...
	while(count<COUNT_LIMIT)
	{
		...
		printf("watch_count(): thread %d Condition signal recieved. Count = %d\n",my_id,count);
	}
 ...
 * Two of these threads increment a "count" variable,
 * while the third thread watches the value of "count".
 * When "count" reached a predefined limit,
 * the waiting thread is signaled by one of the incrementing threads.
 * The waiting thread "awakens" and the modifies count.
...
int main(int argc,char* argv[])
{
	...
	/* Initialie mutex and condition variable objects */
